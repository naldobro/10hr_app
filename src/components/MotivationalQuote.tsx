...
function getBracket(hours: number): number {
  if (hours === 0) return 0;
  if (hours < 2) return 1;
  if (hours < 5) return 2;
  if (hours < 8) return 3;
...
function pickFromBracket(bracket: number): string {
  const pools = [startQuotes, earlyQuotes, midQuotes, advancedQuotes, pushQuotes, legendQuotes];
  const pool = pools[bracket];
  return pool[Math.floor(Math.random() * pool.length)];
}

export default function MotivationalQuote({ workedHours }: MotivationalQuoteProps) {
  const bracket = getBracket(workedHours);
